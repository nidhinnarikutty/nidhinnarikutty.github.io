...
document.getElementById('calBtn').addEventListener('click',()=>{const age=+document.getElementById('calAge').value,gender=document.getElementById('calGender').value,h=+document.getElementById('calHeight').value,w=+document.getElementById('calWeight').value,activity=+document.getElementById('calActivity').value,goal=+document.getElementById('calGoal').value,out=document.getElementById('calResult');if(!age||!h||!w){out.textContent='Please enter age, height and weight.';return}const bmr=10*w+6.25*h-5*age+(gender==='male'?5:-161),tdee=bmr*activity,target=Math.round(tdee+goal),proteinLow=Math.round(w*1.6),proteinHigh=Math.round(w*2);out.innerHTML=`Estimated target: <strong>${target} kcal/day</strong><br>Estimated protein: <strong>${proteinLow}–${proteinHigh} g/day</strong>`});

let step=1;const steps=[...document.querySelectorAll('.form-step')],stepLabel=document.getElementById('stepLabel'),progressBar=document.getElementById('progressBar');function showStep(n){step=n;steps.forEach(s=>s.classList.toggle('active',+s.dataset.step===n));stepLabel.textContent=`Step ${n} of 3`;progressBar.style.width=`${n/3*100}%`}document.querySelectorAll('.next-btn').forEach(btn=>btn.addEventListener('click',()=>{const current=steps.find(s=>+s.dataset.step===step),req=[...current.querySelectorAll('[required]')];if(req.some(i=>i.type==='checkbox'?!i.checked:!i.value)){const bad=req.find(i=>i.type==='checkbox'?!i.checked:!i.value);bad?.focus();return}showStep(Math.min(3,step+1))}));document.querySelectorAll('.prev-btn').forEach(btn=>btn.addEventListener('click',()=>showStep(Math.max(1,step-1))));

const ASSESSMENT_WEBHOOK='https://nidhin281200.app.n8n.cloud/webhook/f13e3bf8-845c-4ded-9447-1fb24dcf2eb2';

document.getElementById('assessmentForm').addEventListener('submit',async e=>{
  e.preventDefault();
  const form=e.currentTarget,fd=new FormData(form),submitBtn=form.querySelector('button[type="submit"]'),originalText=submitBtn.textContent;
  submitBtn.disabled=true;submitBtn.textContent='Sending...';
  const payload={firstName:fd.get('firstName')||'',age:fd.get('age')||'',height:fd.get('height')||'',weight:fd.get('weight')||'',location:fd.get('location')||'',goal:fd.get('goal')||'',training:fd.get('training')||'',experience:fd.get('experience')||'',challenge:fd.get('challenge')||'',whatsapp:fd.get('whatsapp')||'',email:fd.get('email')||'',target:fd.get('target')||'',source:'Physiqo Website - Free Fitness Assessment',pageUrl:window.location.href,submittedAt:new Date().toISOString()};
  try{
    await fetch(ASSESSMENT_WEBHOOK,{method:'POST',mode:'no-cors',headers:{'Content-Type':'text/plain;charset=UTF-8'},body:JSON.stringify(payload),keepalive:true});
    const msg=`Hi Physiqo, I completed my free fitness assessment. My name is ${payload.firstName} and my main goal is ${payload.goal}. I would like to continue.`;
    document.getElementById('waContinue').href=`https://wa.me/919061755675?text=${encodeURIComponent(msg)}`;
    steps.forEach(s=>s.classList.remove('active'));document.querySelector('.form-progress').style.display='none';document.getElementById('formSuccess').classList.add('show');
  }catch(error){
    console.error('Assessment webhook error:',error);alert('We could not send your assessment right now. Please try again.');
